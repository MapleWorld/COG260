...
# E29 => E, 29
# A1 A2 A3 A4, ..., => {A:True}
# {A:TRUE, B:TRUE, C:TRUE, .... G:TRUE} => SIZE = 11
def findChipRowSize(allChip):
    rowDict = {}
    for chip in allChip: # Chip -> E29, A2, A3
        key = chip[0:1] # key = E, A
...
def findChipColumnSize(allChip):
    columnList = []
    for chip in allChip:
        # chip[0:1] abc => a, chip[1:3] => bc 
        # len(chip) => chip = abc => 3 => chip[1:3] 
        key, val = chip[0:1], chip[1:len(chip)] # Key= E, val = 29
        columnList.append(int(val)) # [1,2,3,4,5,38,40,6]
    ...
    for languageIndex in fociData:
        for speakerIndex in fociData[languageIndex]:
            for colorTerm in fociData[languageIndex][speakerIndex]:
                for fociCoordinate in fociData[languageIndex][speakerIndex][colorTerm]: # ["A:29", "E:29"]
                    keyVal = fociCoordinate.split(":") 
                    key = keyVal[0]
                    val = int(keyVal[1])
                    chipHitCountMap[key][val] += 1 # ["A"][29] , A:[0,0,0,...,1,0,0,...,0]
    return chipHitCountMap
...
def readBKDictData(DictDataFilePath):
    ...
    for line in fileHandler:				# for each line in the file
        ...
        # 1	AX	4	?axdar => line.split() => ["1","AX","4","?axdar"]
